	  ...
	  while (true)
		  {
			...
			std::string random_hash_string = picosha2::hash256_hex_string(random_string);

			if (random_hash_string.substr(random_hash_string.size() - end_of_hash.size()) == end_of_hash) 
				{
				  BOOST_LOG_TRIVIAL(info) << "Found correct value '" << random_string << "' with hash '" << random_hash_string << "'";
				} 
			else
				{
